  ...
  try {
    ...
    if (!authToken) {
      try {
        ...
        authToken = (session as any).user?.access_token || (session as any).user?.token || (session as any).user?.sub;
    ...
    // Montar a URL de callback - irá redirecionar direto para a página de integrações
    const callbackUrl = typeof window !== 'undefined' 
      ? `${window.location.origin}/dashboard/integrations`
      : '/dashboard/integrations';
    
    const authorizeUrl = `${apiUrl}/integrations/oauth/authorize?provider=${provider}&redirect_uri=${encodeURIComponent(callbackUrl)}`;
